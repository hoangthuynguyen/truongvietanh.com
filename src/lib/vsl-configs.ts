...
export function getVSLContent(level: VSLLevel, location: string) {
  if (level === 'tieu-hoc') {
    return {
      ...
      ctaPrimary: 'NHẬN TƯ VẤN NGAY — Lộ trình Trại Hè Lion Camp cho bé',
  ...
  if (level === 'thcs') {
    return {
      ...
      ctaPrimary: 'NHẬN TƯ VẤN NGAY — Tìm lộ trình phù hợp cho con',
  ...
  return {
    ...
    ctaPrimary: 'NHẬN TƯ VẤN HƯỚNG NGHIỆP NGAY CHO CON',
